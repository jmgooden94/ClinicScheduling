...
public class MySqlUtils
{
            ...
            SQLException
    {
        ...
        String sql = "SELECT clinic.appointment.appt_type FROM clinic.appointment WHERE clinic.appointment.appt_type IS NOT NULL " +
                "AND clinic.appointment.appt_type != \"" + SpecialType.PROVIDER_UNAVAILABLE.toString() + "\" AND " +
                "clinic.appointment.start_time BETWEEN ? AND ? AND clinic.appointment.status IS NULL";
        PreparedStatement ps = connection.prepareStatement(sql);
        Timestamp begin = new Timestamp(start.getTimeInMillis());
        Timestamp stop = new Timestamp(end.getTimeInMillis());
        ps.setTimestamp(1, begin);
            ...
            throws SQLException
    {
        ...
        String sql = "SELECT clinic.appointment.status FROM clinic.appointment WHERE clinic.appointment.status IS NOT NULL " +
                "AND clinic.appointment.appt_type != \"" + SpecialType.PROVIDER_UNAVAILABLE + "\" AND " +
